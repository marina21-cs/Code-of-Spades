...
import { storageDeleteItem, storageGetItem, storageSetItem } from './secureStorage';
...
export async function saveProfile(profile: LearningProfile): Promise<void> {
  await storageSetItem(PROFILE_KEY, serializeProfile(profile));
  emitProfileChange();
}
...
export async function isFirstRun(): Promise<boolean> {
  const done = await storageGetItem(FIRST_RUN_DONE_KEY);
  return done == null;
}
...
export async function markFirstRunComplete(): Promise<void> {
  await storageSetItem(FIRST_RUN_DONE_KEY, '1');
}
...
export async function resetProfile(): Promise<void> {
  await storageDeleteItem(PROFILE_KEY);
